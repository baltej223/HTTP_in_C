...
SERVER Create_Server() {
  ...
  s.body = malloc(sizeof(struct body_fns));
  if (s.body == NULL)
      exit(EXIT_FAILURE);
  // body_fn
  s.body->whatever = &whatever;

s.cntx = malloc(sizeof(struct context));
if (s.cntx == NULL)
    exit(EXIT_FAILURE);
  // cntx
  s.cntx->reply_same_to_all_reqs = false;
  // s.cntx->global_response = NULL;
  return s;
}
...
void whatever(SERVER *s, VECTOR *res) {
  s->cntx->reply_same_to_all_reqs = true;
  s->cntx->global_response = create_string_vector();
  append_vector_to_vector(&(s->cntx->global_response), res);
}


VECTOR what_to_return_to_client(SERVER *s, struct request *req) {
  VECTOR to_reply = create_string_vector(); 
  if (s->cntx->reply_same_to_all_reqs) {
    append_vector_to_vector(&to_reply, &s->cntx->global_response);
    return to_reply;
  } 
}
...
void run(SERVER* s) {
  ...
  server.sin_port = htons(s->PORT); // htons means: host to network store
  ...
  for (int i = 0; i < 10000000;
    ...
    RESPONSE res = generate_response(req, s); // its going to give response
